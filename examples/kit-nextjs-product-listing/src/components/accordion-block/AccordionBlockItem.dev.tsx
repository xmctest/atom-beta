...
export const AccordionBlockItem = ({
  ...
  const heading = getFieldValue(child?.heading);
  const description = getFieldValue(child?.description);
    ...
    <>
      ...
      >
        <AccordionTrigger className="font-heading flex w-full justify-between py-4 text-left text-base font-medium">
          {heading && <Text field={heading} className="font-heading text-left text-base font-medium" />}
        </AccordionTrigger>
        <AccordionContent>
          <div className="font-body py-4 pt-2 text-base font-medium">
            {description && <RichText tag="div" field={description} />}
          </div>
        </AccordionContent>
      </AccordionItem>
    </>
